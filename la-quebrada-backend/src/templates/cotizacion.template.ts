...
<style>
  ...
  .header h1 { color: #093509; margin: 0; font-size: 22px; }
  ...
  .info-box h3 { margin: 0 0 8px; font-size: 13px; color: #093509; }
  .info-box p { margin: 3px 0; }
  table { width: 100%; border-collapse: collapse; margin-bottom: 15px; }
  th { background: #093509; color: white; text-align: left; padding: 6px 8px; font-size: 11px; }
  td { padding: 6px 8px; border-bottom: 1px solid #eee; }
  .num { text-align: right; }
  .seccion-titulo { background: #f0f0f0; font-weight: bold; padding: 6px 8px; margin-top: 10px; }
  .totales { width: 300px; margin-left: auto; margin-top: 10px; }
...
<body>
  <div class="header">
    ...
    <p>Cotización — Versión ${d.version}</p>
  </div>

  <div class="info-boxes">
    <div class="info-box">
      <h3>Cliente</h3>
      <p><strong>${d.clienteNombre}</strong></p>
      <p>${d.clienteTelefono || '—'}</p>
    </div>
    <div class="info-box">
      <h3>Evento</h3>
      <p>${d.eventoTipo || 'Evento'} — ${d.eventoFecha}</p>
      <p>${d.eventoSalones}</p>
    </div>
    <div class="info-box">
      <h3>Cotización</h3>
      <p>Fecha: ${d.fechaCotizacion}</p>
      <p>Vigencia: ${d.vigenciaDias} días</p>
      ${d.vendedor ? `<p>Vendedor: ${d.vendedor}</p>` : ''}
    </div>
  </div>

  ${d.menus.length > 0 ? `
    <div class="seccion-titulo">Menú</div>
    <table>
  ...
  <div class="footer">
    <p>Vigencia de esta cotización: ${d.vigenciaDias} días desde la fecha de emisión.</p>
    <p>Para confirmar el evento se requiere un abono no reembolsable. Consulte condiciones con su vendedor.</p>
  </div>
</body>
</html>
  `;
